...
def test_run_all_evaluations_uses_only_instance_zero(config_small, tmp_path):
    from fedshop.evaluate import run_all_evaluations

    config_small.generation.n_query_instances = 2
    config_small.evaluation.engines = {
        "pyfedx": config_small.evaluation.engines.get("fedshop-go")
    }
    gen_dir = tmp_path / "generation" / "q01"
    gen_dir.mkdir(parents=True)
    ...
    def fake_run_evaluation(**kwargs):
        ...
        stats = (
            tmp_path / "evaluation" / kwargs["engine_name"] / kwargs["query_name"]
            / f"instance_{kwargs['instance_id']}" / f"batch_{kwargs['batch_id']}"
            / f"attempt_{kwargs['attempt']}" / "stats.csv"
        )
        stats.parent.mkdir(parents=True, exist_ok=True)
        stats.write_text("exec_time\n0.1\n")
        return stats

    with patch("fedshop.evaluate.run_evaluation", side_effect=fake_run_evaluation):
        run_all_evaluations(config_small, tmp_path, engine_filter="pyfedx", query_filter="q01")
