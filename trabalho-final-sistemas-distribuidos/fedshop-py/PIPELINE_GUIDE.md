...
```text
inputs/config/config_small.yaml
```
...
The pipeline also maintains a structured DuckDB artifact database:

```text
fedshop.duckdb
```

DuckDB is the compact, queryable view of the run. The legacy files remain the
compatibility surface for engines and scripts.
...
The default config file is `inputs/config/config_small.yaml`.

It is loaded by `fedshop.config.load_config()`. The file is intentionally
compact; the loader derives the internal Virtuoso, WatDiv schema, proxy, and
federation-member structures required by the pipeline.
...
| Key | Meaning |
|---|---|
| `profile` | Human-readable run profile. |
| `use_docker` | Controls whether generated endpoint URLs use Docker host conventions where needed. |
| `scale` | Batch, source, product, and compact WatDiv scale settings. |
| `queries` | Query-template location and single-instance setting. |
| `inputs` | Query/template/RDF input locations and transient workdir. |
| `services` | Local Virtuoso and proxy ports. |
| `evaluation` | Attempts, timeout, and supported engines. |

### Compact Settings

Default values in `inputs/config/config_small.yaml`:
...
inputs:
  ...
  dataset: inputs/product-dataset
  workdir: data
services:
  virtuoso_port: 8890
  proxy_port: 5555
...
|---|---|---|
| `scale.batches` | Number of cumulative federation batches. | `2` |
| `scale.members_per_batch` | Number of vendors and ratingsites added per batch. | `10` |
| `scale.product_count` | Shared product catalog size. | `20000` |
| `queries.instances` | Concrete query instances per template. The simplified pipeline uses one. | `1` |
| `queries.dir` | Query templates and `.const.json` files. | `inputs/queries` |
| `inputs.templates` | WatDiv templates. | `inputs/templates` |
| `inputs.dataset` | Materialized vendor/ratingsite `.nq` files. | `inputs/product-dataset` |
| `inputs.workdir` | Transient generated files and proxy mappings. | `data` |
| `services.virtuoso_port` | Local Virtuoso port. | `8890` |
| `services.proxy_port` | Local proxy port. | `5555` |

The simplified pipeline materializes one canonical query instance per template
regardless of the legacy `n_query_instances` value. It keeps `instance_0` in the
file layout for compatibility:

```text
benchmark/generation/q01/instance_0/injected.sparql
benchmark/evaluation/pyfedx/q01/instance_0/batch_0/attempt_0/
```

Batches change the federation size, not the query instance. The same
`injected.sparql` is executed against every batch, while reference answers stay
batch-specific as `results-batchN.csv`.

## DuckDB artifacts

`fedshop.duckdb` uses three schemas:
...
| Table | Purpose |
|---|---|
| `input.config` | Resolved run scale, paths, and a compact `config_json`. |
| `input.rdf` | Generated N-Quads normalized as subject, predicate, object plus source metadata. |
| `input.queries` | One instantiated query per template; no `instance_id` column. |
| `output.executions` | One row per engine/query/batch/attempt with status, timing, and network counters. |
| `output.results` | Reference and engine result rows as JSON bindings. |
| `output.source_selection` | Selected sources per triple pattern. |
| `output.metrics` | Final correctness, timing, network, and source-selection metrics. |
| `meta.artifacts` | Reserved for exported legacy artifact metadata. |

`input.config.config_json` intentionally contains only details that are not
usually filtered directly: model/docker path defaults, Virtuoso/proxy endpoints,
the two supported engines (`pyfedx` and `fedshop-go`), and compact WatDiv
template defaults. Scale values such as batches, members per batch, attempts,
and product count are first-class columns on `input.config`.
