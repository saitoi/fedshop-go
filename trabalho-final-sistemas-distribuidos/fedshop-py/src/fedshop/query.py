...
def generate_queries_for_template(
    ...
    *,
    ...
    # Step 3+4: instantiate and execute the single simplified query instance.
    rows = wvs_df.to_dict(orient="records")
    persisted_query = False
    for instance_id, row in enumerate(rows[:1]):
...
def _persist_query_artifact(
    *,
    ...
    batch_id: int,
    ref_results_path: Path,
) -> None:
    try:
        from .artifacts import ArtifactStore, default_artifact_db

        bench_dir = output_dir.parent.parent if output_dir.parent.name == "generation" else None
        db_path = default_artifact_db(config, bench_dir)
        store = ArtifactStore(db_path)
        config_id = store.persist_config(
            config,
            bench_dir=bench_dir or db_path.parent,
            artifact_db=db_path,
        )
        store.persist_query(
            config_id=config_id,
            query_id=template_path.stem,
            selection_batch_id=0,
            template_path=template_path,
            const_path=const_path,
            template_sparql=query_text,
            const_json=const_info,
            selected_values=selected_values,
            injected_sparql=injected,
        ...
        store.persist_results_csv(
            ...
            batch_id=batch_id,
            attempt=None,
            csv_path=ref_results_path,
        )
    except Exception:
        # DuckDB persistence is auxiliary; legacy artifacts remain authoritative
        # for engine execution and compatibility.
        pass
