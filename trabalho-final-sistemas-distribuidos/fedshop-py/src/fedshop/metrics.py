...
def compute_full_metrics(
    ...
    for sf in sorted(bench_dir.glob("evaluation/*/*/*/*/*/stats.csv")):
        ...
        records.append({
            ...
            "request_bytes": _f("request_bytes"),
            "net_req_count": _f("net_req_count"),
            "net_total_time": _f("net_total_time"),
            "net_mean_time": _f("net_mean_time"),
            "net_p50_time": _f("net_p50_time"),
            "net_p95_time": _f("net_p95_time"),
            "endpoints_contacted": _f("endpoints_contacted"),
            "endpoint_load_imbalance": _f("endpoint_load_imbalance"),
            "tpwss": tpwss,
            "avg_rwss": avg_rwss,
            "min_rwss": min_rwss,
            "max_rwss": max_rwss,
    ...
    _persist_metrics_artifacts(config, bench_dir, df)
    return df
...
def compute_metrics(
    ...
    _persist_metrics_artifacts(config, Path(output_path).parent, metrics_df)
    return metrics_df


def _persist_metrics_artifacts(
    config: BenchmarkConfig,
    bench_dir: Path,
    ...
    try:
        ...
        config_id = store.persist_config(config, bench_dir=bench_dir, artifact_db=db_path)
        store.persist_metrics(config_id, metrics_df)
    except Exception:
        pass
