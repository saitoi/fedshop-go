	...
	for i := 0; i < len(remaining); i++ {
		...
		if filterOnlyVars != nil && len(pendingFilters) > 0 {
			if groupLen := detectFilterGroup(remaining[i:], filterOnlyVars, varCount, rows); groupLen >= 2 {
				...
				rows, pendingFilters, handled, s, err = e.executeFilterGroup(
				...
				// Only skip the grouped triples when the group was actually consumed;
				// otherwise (e.g. the filter also references a variable bound later, like
				// an OPTIONAL var) fall through and process the triples normally.
				if handled {
					if debugExec {
...
// The boolean return reports whether the group was actually consumed; when false
// (e.g. no pending filter is fully coverable because it also references a variable
// bound only later, like an OPTIONAL var), the caller must fall back to processing
// the triples normally instead of skipping them.
func (e *Executor) executeFilterGroup(
...
) ([]Binding, []string, bool, Stats, error) {
	stats := Stats{}
	if len(groupTriples) == 0 || len(rows) == 0 {
		return rows, pendingFilters, false, stats, nil
	}
	...
	for tpIdx, tp := range groupTriples {
		...
		stats = addStats(stats, s)
		if err != nil {
			return nil, pendingFilters, true, stats, err
		}
		union = distinct(union, nil)
		if debugExec {
		...
		if len(passedThisTriple) == 0 {
			return rows[:0], removeConsumed(pendingFilters, consumableSet), true, stats, nil
		}
	...
	return kept, removeConsumed(pendingFilters, consumableSet), true, stats, nil
}
