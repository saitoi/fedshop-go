...
func TestParseFedShopTemplates(t *testing.T) {
	...
	if len(queries) == 0 {
		// cópia local do trabalho final: templates ficam em fedshop-py/inputs/queries
		queries, err = filepath.Glob(filepath.Join("..", "..", "..", "fedshop-py", "inputs", "queries", "q*.sparql"))
		if err != nil {
			t.Fatalf("glob templates (inputs): %v", err)
		}
	}
	if len(queries) != 12 {
