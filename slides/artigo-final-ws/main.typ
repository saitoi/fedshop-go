...
#figure(
  table(
    ...
    [RSA],
      [Broadcast (SERVICE pré-atribuído)],
      [—],
      [Hash join local],
      [Não],
      [Go],
    [FedShop-Go],
      [ASK + cache por triple pattern],
      [—],
      [Hash join / bound join],
      [Não],
      [Go],
    [FedX],
      [ASK exclusivo por triple pattern],
      [Heurísticas de join order],
      [Bound join (VALUES/FILTER)],
      [Não],
      [Java/RDF4J],
    [PyFedX],
      [ASK (sem cache)],
      ...
      [Python],
    [SPLENDID],
      [Estatísticas VOID (índices invertidos)],
      [VOID: triple count + partições],
      [Hash join (ordenado por custo VOID)],
      [Sim (VOID)],
      [Java],
    [SemaGrow],
      [Metadados instance-level + VOID],
      [Modelo de custo com cardinalidade],
      [Bind / merge / hash join (async)],
      [Sim (metadados)],
      [Java/Sesame],
    table.hline(stroke: .6pt),
  ),
